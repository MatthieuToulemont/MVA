                    ...
                    const Image<byte>& im2, int i2,int j2,float m2)
{
    ...
    for(int i = -win; i<win; i++){
        for(int j = -win; j<win; j++){
            ...
            var1 += pow((im1(i1+i, j1+j)-m1), 2);
            var2 += pow((im2(i2+i, j2+j)-m2), 2);
        }
    }
    dist = dist/(pow(var1,0.5)*pow(var2,0.5)+EPS);
    return dist;
}
...
void BestNccD(Image<byte> im1, Image<byte> im2, float dmin, float dmax,int shift, int x, int y,  int& bestDx, float& bestNcc, int& prop){
    float ncc;
    for (int dx= dmin;dx<=dmax+1; dx++){
        if(x+dx +shift>win ){
            // Compute Centered Correlation
            ncc = ccorrel(im1, x, y, im2, x+dx+shift, y);
            if(ncc > bestNcc){
                // If big enough update best.
                bestNcc = ncc;
                bestDx = dx+shift;
            }
        }
...
void UpdateSeeds(int x, int y, float &bestncc, int& bestdx, Image<int> &disp, Image<bool> &seeds,
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<im1.height() && y+win<im2.height(); y++)
        for(int x=win; x+win<im1.width(); x++) {
            ...
            if (x+dMin>0 && x+dMax<im2.width()){
                float bestncc = -10;
                int bestdx;
                int shift = 0;
                int prop = 0;
                // Compute best NCC and Best Dx :
                BestNccD(im1, im2, dMin, dMax, shift, x, y,  bestdx, bestncc, prop);
                if(bestncc>nccSeed){

                    // Add Seed
                    UpdateSeeds(x,y,bestncc, bestdx, disp, seeds, Q);
                }
            }
        }
}
        ...
        for(int i=0; i<4; i++) {
            ...
            if(0<=x-win && 0<=y-win &&
               ...
               ! seeds(x,y)) {
                ...
                int prop = 1;
                // Compute best NCC and Best Dx :
                BestNccD(im1, im2, dmin, dmax, shift, (int)x, (int)y,  bestdx, bestncc, prop);
                // Add Seed
                UpdateSeeds((int)x,(int)y,bestncc, bestdx, disp, seeds, Q);
